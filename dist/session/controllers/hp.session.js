...
HpSession.startSession = core_1.CatchAsync.wrap(async (req, res, next) => {
    ...
    await session.save();
    return core_1.responseHandler.success(res, core_1.HttpStatus.OK, "Session started successfully!", session);
});
...
HpSession.createFollowUpSession = core_1.CatchAsync.wrap(async (req, res, next) => {
    ...
    const { time } = req.body;
    const parentSession = await core_1.Session.findOne({ where: { id: parentSessionId } });
    if (!parentSession) {
